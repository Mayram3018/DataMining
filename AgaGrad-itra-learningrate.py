...
def adagrad_method(similarity, max_iter=10, tol=1e-6, learning_rate=0.01):
    # Initialize a counter for iterations, a cache for squared gradients, and a list to store learning rates
    i = 0
    ...
    while i < max_iter:
        ...
        # Store the learning rate at each iteration
        learning_rates.append(adaptive_lr)

        # Increment the counter
        i += 1

    # Return the optimized similarity matrix, the final objective value, and the learning rates
    return similarity, f, learning_rates


# Run AdaGrad method on the initial similarity matrix
similarity_opt, f_opt, learning_rates = adagrad_method(similarity)
